...
const supportTicketsStore: SupportTicket[] = [
  {
    ...
    messages: [
      ...
      },
      {
        id: 'tmsg-2',
        sender: 'ai_assistant',
        senderName: 'ShopSense Support AI',
...
export function createSupportTicket(
  ...
  // Generate immediate smart AI support triage response
  ...
  const aiMsg: SupportTicketMessage = {
    id: `tmsg-${Date.now() + 1}`,
    sender: 'ai_assistant',
    senderName: 'ShopSense Support Assistant',
    content: aiReplyText,
    timestamp: now
  };
...
export function addMessageToTicket(ticketId: string, sender: 'user' | 'support_agent' | 'ai_assistant', senderName: string, content: string): SupportTicket | null {
