...
import { BuyBoxSellersSection } from './BuyBoxSellersSection';
...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const handleFetchDecisionNarrative = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setDecisionNarrative(data.explanation);
      onTrackEvent('explain_recommendation_view', { productId: product.id });
    } catch (err) {
      console.error('Explain failed:', err);
      setDecisionNarrative('This product was ranked highly due to strong semantic alignment with your tech requirements and brand preferences.');
    } finally {
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col justify-between">
              <div>
                ...
                {/* 🏆 Amazon-Grade Buy Box & Multi-Seller Winner */}
                <div className="mt-3">
                  <BuyBoxSellersSection
                    productId={product.id}
                    currency={currency}
                  />
                </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-950 border border-indigo-900/60 shadow-lg space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <button
                onClick={handleFetchDecisionNarrative}
              ...
              >
                {loadingNarrative ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                <span>{loadingNarrative ? 'Synthesizing...' : 'Generate Decision Narrative'}</span>
              </button>
            </div>

            {/* Model Decision Narrative Box */}
            {decisionNarrative && (
              <div className="p-3.5 rounded-xl bg-indigo-950/40 border border-indigo-800/50 text-xs text-indigo-200 leading-relaxed animate-in fade-in">
                <span className="font-semibold text-cyan-300 block mb-1">Model Decision Narrative:</span>
                {decisionNarrative}
              </div>
            )}
          ...
          {/* 🛍️ Amazon "Frequently Bought Together" & Smart Ecosystem Bundle */}
          ...
          {/* ⭐ Verified Customer Reviews & Aspect Sentiment */}
