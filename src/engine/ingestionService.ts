...
export function cosineSimilarity(a: number[], b: number[]): number {
  ...
  return Math.round((dot / (Math.sqrt(normA) * Math.sqrt(normB))) * 100) / 100;
}

export function getProjectedCatalogEmbeddings(userEmbedding?: number[]) {
  return dynamicProductCatalog.map((product) => {
    const emb = product.embedding && product.embedding.length >= 8
      ? product.embedding
      : generateProductEmbedding(product.title, product.category, product.tags, product.description, product.priceINR);

    // Principal Component 1: Compute (idx 0) + Gaming (idx 2) - Value (idx 7) - Portability (idx 1)
    const pc1 = ((emb[0] || 0.5) * 60 + (emb[2] || 0.5) * 45) - ((emb[7] || 0.5) * 35 + (emb[1] || 0.5) * 30);
    // Principal Component 2: Audio (idx 3) + Camera (idx 5) - Battery (idx 4) - Compute (idx 0)
    const pc2 = ((emb[3] || 0.5) * 55 + (emb[5] || 0.5) * 40) - ((emb[4] || 0.5) * 30 + (emb[0] || 0.5) * 20);

    const x = Math.max(-85, Math.min(85, Math.round(pc1 * 1.5)));
    const y = Math.max(-85, Math.min(85, Math.round(pc2 * 1.5)));

    const sim = userEmbedding ? cosineSimilarity(emb, userEmbedding) : 0.85;
