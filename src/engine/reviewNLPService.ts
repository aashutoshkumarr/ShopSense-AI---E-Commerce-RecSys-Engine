...
export function getProductNLPSummary(productId: string): ProductNLPSummary {
  ...
  const aiExecutiveSummary = `${topProStr}. Sentiment analysis reveals an overwhelmingly positive response with ${Math.round((posCount / Math.max(1, totalReviews)) * 100)}% satisfaction rating across verified purchasers.`;
